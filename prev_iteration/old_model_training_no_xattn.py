...
def build_model(sequence_length_1h, sequence_length_1d, num_features_1h, num_features_1d,
    ...
    for i, (input_dim, output_dim) in enumerate(zip(ticker_info_input_dim, embedding_dims)):
        embedding = Embedding(input_dim=input_dim + 1, output_dim=output_dim, name=f'ticker_info_embedding_{i}')(ticker_info_input[:, i])
...
def main_model_training():
    ...
    h5f_1h = h5py.File('/content/1h_train_data_20240926_052634.h5', 'r')  # Replace with actual filename
    ...
    h5f_1d = h5py.File('/content/1d_train_data_20240926_052950.h5', 'r')  # Replace with actual filename
    ...
    y_train_seq = y_train_seq_1h[:min_len]  # Using 1h labels; ensure labels are consistent
    ticker_indices = ticker_indices_1h[:min_len]  # Assuming tickers are aligned

    # Load the label encoders for OHLCV data (Ticker Encoder)
    with open('full_ohlcv_label_encoders2.pkl', 'rb') as f:
        ticker_label_encoders = pickle.load(f)

    # Get the LabelEncoder for 'Ticker'
    ticker_le = ticker_label_encoders['Ticker']
    ...
    # Load label encoders and category mappings for ticker info data
    with open('label_encoders_ticker.pkl', 'rb') as f:
        ticker_info_label_encoders = pickle.load(f)
    with open('category_mappings_ticker.pkl', 'rb') as f:
        ticker_info_category_mappings = pickle.load(f)
    ...
    for col in ticker_categorical_cols:
        le = ticker_info_label_encoders[col]
        ticker_info_data[col] = le.transform(ticker_info_data[col])

    # Extract ticker info data for the tickers in our sequences
    valid_tickers = [ticker for ticker in ticker_symbols if ticker in ticker_info_data.index]
    ...
    ticker_info_input_dim = [max(ticker_info_data[col].max(), 0) for col in ticker_categorical_cols]
    ...
    model.save('hierarchical_transformer_model2.keras')
    print("Model saved to hierarchical_transformer_model2.keras")
